...
class ImportUtils extends SugarBean
{
    ...
    {
        file_put_contents('test_cstm.txt', print_r($join_fields,1));
        ...
        if($row = $db->fetchByAssoc($result))
        {
            ...
            $sql_upd .= " WHERE `id_c` = '{$uuid}'";
            file_put_contents(__DIR__."/cstm_upload.sql", $sql_upd);
...
//        );
    ...
    static public function checkIfExists($current_table, $fields, $is_related, $parent_id,$parent_module, $relate_id, $this_module_rel)
    {
        ...
        foreach ($fields['search_fields'] as $keyf => $valuef)
        {
            ...
            {
                if($valuef['func'] == 'eq')
                    $sql_d .= " AND `{$current_table}`.`{$keyf}` = '{$valuef['cell']}' ";
                else
                    $sql_d .= " AND `{$current_table}`.`{$keyf}` LIKE '%{$valuef['cell']}%' ";
                }
            else
            {
                $email_join = " LEFT JOIN `email_addr_bean_rel` ON `email_addr_bean_rel`.bean_id = `{$current_table}`.id "
                        . " LEFT JOIN `email_addresses` ON `email_addresses`.id = `email_addr_bean_rel`.email_address_id ";
                $email_where = " AND `email_addresses`.email_address = '{$valuef['cell']}' ";
            }
            
            //$sql .= " AND `{$keyf}` = '{$valuef}' ";
        }
        
        $sql .= $email_join. "WHERE 1=1 ".$email_where.$sql_d;
        if($is_related)
        {
    ...
    static public function checkIfContactExists($current_table, $fields, $is_related, $parent_id,$parent_module, $relate_id, $this_module_rel)
    {
        ...
        $sql .= $email_join. " WHERE 1=1 "
             . " AND jt.{$parent_table_id} = '{$parent_id[$parent_module]}' "
             . " AND jt.deleted = 0 ".$email_where.$sql_d;
