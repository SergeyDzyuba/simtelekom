...
','',$cell);
                    $cell = trim($cell);
					//Проверка на enum
					$current_mod_type = loadBean($key);
					if(isset($current_mod_type->field_defs[$field_map[$cell_number]['name']]) && $current_mod_type->field_defs[$field_map[$cell_number]['name']]['type'] == 'enum')
					{
						$options_name = $current_mod_type->field_defs[$field_map[$cell_number]['name']]['options'];
						foreach ($app_list_strings[$options_name] as $app_key => $app_value)
						{
							if($app_value == $cell)
							{
								$cell = $app_key;
