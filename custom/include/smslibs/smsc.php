...
class SMSC
{
	...
	function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = false, $query = "", $files = array())
	{
		...
		$str = "cost=3&phones=".urlencode($phones)."&mes=".urlencode($message).
						"&translit=$translit&id=$id".($format > 0 ? "&".$formats[$format] : "").
						($sender === false ? "" : "&sender=".urlencode($sender)).
						($time ? "&time=".urlencode($time) : "").($query ? "&$query" : "");
		file_put_contents('sms.log', "Ошибка № {$m[0]} '{$str}' \n", FILE_APPEND);
		...
		if (SMSC_DEBUG) {
			if ($m[1] > 0){
				...
				file_put_contents('sms.log', "Сообщение отправлено успешно. ID: $m[0], всего SMS: $m[1], стоимость: $m[2], баланс: $m[3].\n", FILE_APPEND);
			}
			else {
				$error_text = $errors[-$m[1]];
				echo "Ошибка №", -$m[1], $m[0] ? ", ID: ".$m[0] : "", "'{$error_text}' \n";
				file_put_contents('sms.log', "Ошибка № {$m[0]} '{$error_text}' \n", FILE_APPEND);
			}
		}
