...
class XlsExUtils
{
	...
	public static function get_related($module_name)
	{
		...
		if(isset($GLOBALS['beanList'][$module_name]))
	...
	public static function getStandartField($bean, $field_name, $field_value)
	{
		...
		if($bean->module_name=='Appeal'){
			if(in_array($field_name, array('source', 'type', 'theme', 'subtheme'))){
				include_once('custom/modules/Home/utils.php');
				return $field_value = LinkedUtils::detail($field_value);
			}
		}
